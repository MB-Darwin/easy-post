...
function Faceted<Multiple extends boolean = false>(props: FacetedProps<Multiple>) {
  ...
  return (
    <FacetedContext value={contextValue}>
...
function FacetedTrigger(props: React.ComponentProps<typeof Popover.Trigger>) {
  const { className, children, ...triggerProps } = props;

  return (
    <Popover.Trigger {...triggerProps} className={cn('justify-between text-left', className)}>
      {children}
    </Popover.Trigger>
  );
}
...
function FacetedBadgeList(props: FacetedBadgeListProps) {
  ...
  return (
    <div {...badgeListProps} className={cn('flex flex-wrap items-center gap-1', className)}>
      {values.length > max ? (
        <Badge color="secondary" className={cn('rounded-sm px-1 font-normal', badgeClassName)}>
...
function FacetedContent(props: React.ComponentProps<typeof Popover.Content>) {
...
const FacetedInput = Command.Input;

const FacetedList = Command.List;

const FacetedEmpty = Command.Empty;

const FacetedGroup = Command.Group;
