      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 flex items-center gap-2">
            Welcome back! You have <span className="font-semibold text-[#3C48F6]">3 posts</span> scheduled for today.
          </p>
        </div>
        <div className="flex gap-3">
            <Button variant="outline" className="hidden sm:flex rounded-full border-slate-300 text-slate-600 hover:text-[#3C48F6] hover:bg-blue-50 hover:border-blue-200 transition-all">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" className="text-slate-400 hover:text-[#3C48F6]">View Calendar</Button>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                ...
                <div className="p-3 bg-slate-50 text-center">
                    <button className="text-sm font-medium text-[#3C48F6] hover:underline">View all upcoming posts</button>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-6">
             ...
             <div className="bg-gradient-to-br from-[#3C48F6] to-[#6366F1] rounded-2xl p-6 text-white shadow-xl shadow-blue-500/20 relative overflow-hidden group">
                ...
                <h3 className="relative z-10 text-blue-100 font-medium mb-1">Total Impressions</h3>
             ...
             <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm">
                <h3 className="font-semibold text-slate-700 mb-4">Top Performing Platform</h3>
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div className="ml-auto font-bold text-emerald-600 text-sm">+14%</div>
                </div>
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm hover:shadow-md hover:border-[#3C48F6]/30 transition-all duration-300 cursor-default">
      <div className="flex justify-between items-start mb-4">
        ...
        {trend && (
             <div className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${trendUp ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
...
function QueueItem({ platform, time, text, status }: any) {
  ...
  return (
    <div className="group flex flex-col sm:flex-row sm:items-center gap-4 p-4 hover:bg-slate-50/80 transition-colors relative">
      <div className={`hidden sm:flex h-10 w-10 shrink-0 items-center justify-center rounded-full shadow-sm ${bgColors[platform]}`}>
        {icons[platform]}
      </div>
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 mb-1">
            <div className={`sm:hidden h-5 w-5 rounded-full flex items-center justify-center ${bgColors[platform]}`}>
                 {icons[platform]}
            </div>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase tracking-wide ${statusColors[status] || 'bg-slate-100'}`}>
                {status}
            </span>
      ...
      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <Button variant="ghost" size="sm" className="h-8 text-slate-400 hover:text-slate-700">Edit</Button>
